...
{
    public partial class WmsAutoStockInErrorForm : MesBaseForm
    {
        private readonly IFactoryService _factoryService;
        private readonly IWmsAutoStockInLogService _wmsAutoStockInLogService;
        public WmsAutoStockInErrorForm(IFactoryService factoryService, IWmsAutoStockInLogService wmsAutoStockInLogService)
        {
            InitializeComponent();
            InitializeTable();
            _factoryService = factoryService;
            _wmsAutoStockInLogService = wmsAutoStockInLogService;
        }
        ...
        private async Task<int> LoadDataAsync()
        {
            try
            {
                ...
                var result = await _wmsAutoStockInLogService.GetFailListAsync(factory.FactoryCode, keyword);
                if (result != null)
                {
                    TableControl.DataSource = result.Select(x => new WmsAutoStockInErrorViewModel(x)).ToList();
                    return result.Count();
                }
                else
                {
        ...
        private async void TableControl_CellButtonClick(object sender, AntdUI.TableButtonEventArgs e)
        {
            ...
            {
                if (e.Record is WmsAutoStockInErrorViewModel data)
                {
                    var updateDto = new WmsAutoStockInLogUpdateDto()
                    {
                        Id = data.Id,
                    };
                    if (e.Btn.Id == "reentry")
                    ...
                    await _wmsAutoStockInLogService.UpdateAsync(updateDto);
                    await LoadDataAsync();
                }
    ...
    public class WmsAutoStockInErrorViewModel : AntdUI.NotifyProperty
    {
        public WmsAutoStockInErrorViewModel(WmsAutoStockInLogDto dto) 
        {
